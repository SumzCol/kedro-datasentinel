...
from kedro_datasentinel.utils import try_load_obj_from_class_paths, try_load_obj
...
class CheckConfig(BaseModel):
    ...
    def _create_check_obj(self, name: str) -> AbstractCheck:
        ...
        class_obj = try_load_obj_from_class_paths(
            class_paths=[
                class_path,
                f"dataguard.validation.check.{class_path}"
            ]
        )
        ...
        check_obj_args = self.model_dump(
            exclude={"rules", "mode", "type"}
        )
...
class ValidationWorkflowConfig(BaseModel):
    ...
    data_asset: Optional[str] = None
    data_asset_schema: Optional[str] = None
    check_list: dict[str, CheckConfig]
    result_stores: Optional[list[str]] = Field(default_factory=list)
    notifiers_by_events: Optional[dict[NotifyOnEvent, list[str]]] = Field(
    ...
    def has_online_checks(self) -> bool:
        return any([
            check_conf.mode == Mode.ONLINE or check_conf.mode == Mode.BOTH
    ...
    def has_offline_checks(self) -> bool:
        return any([
            check_conf.mode == Mode.OFFLINE or check_conf.mode == Mode.BOTH
            for check_conf in self.check_list.values()
        ])
        ...
        return ValidationWorkflow(
            data_validation=DataValidation(
                ...
                data_asset=MemoryDataAsset(
                    name=self.data_asset or dataset_name,
                    schema=self.data_asset_schema,
                    data=data
                ),
                check_list=[
                    check_conf.create_check(name=name)
                    for name, check_conf in self.check_list.items()
                    if check_conf.mode == mode or check_conf.mode == Mode.BOTH
                ],
            ),

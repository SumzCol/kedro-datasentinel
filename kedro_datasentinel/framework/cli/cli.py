...
from kedro_datasentinel.core import Mode, DataValidationConfigError
...
@click.option(
    ...
    "-e",
    default="local",
    help="The name of the kedro environment where the 'datasentinel.yml' should be created. Default to 'local'",
)
@click.option(
    "--force",
    "-f",
...
def init(env: str, force: bool):
    ...
    config_template = resources.read_text(
        "kedro_datasentinel.template", filename
    )
    ...
    with KedroSession.create(
        ...
        try:
            ...
            raise DataValidationConfigError(
                ...
                f"could not be parsed, please verify that it has a valid structure: {str(e)}"
            ) from e
